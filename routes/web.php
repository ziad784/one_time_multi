...
Route::get('wating', [RegisteredUserController::class, 'wating']);
Route::get('markread', [RegisteredUserController::class, 'markread']);
  // vendor profile
  Route::get('vendorprofile/{id}', [VendorController::class,'vendorProfile']);
...
Route::prefix('/admin')->namespace('App\Http\Controllers\Admin')->group(function() {
    Route::match(['get', 'post'], 'login', 'AdminController@login'); // match() method is used to use more than one HTTP request method for the same route, so GET for rendering the login.php page, and POST for the login.php page <form> submission (e.g. GET and POST)    // Matches the '/admin/dashboard' URL (i.e. http://127.0.0.1:8000/admin/dashboard)
        ...
        Route::get('register/step_2', [RegisteredUserController::class, 'step2']);

        Route::get('register/step_3', [RegisteredUserController::class, 'step3']);
    
        Route::get('register/step_4', [RegisteredUserController::class, 'step4']);
        
        Route::post('register', [RegisteredUserController::class, 'store']);
        
        Route::post('validation', [RegisteredUserController::class, 'validation']);
    ...
    Route::group(['middleware' => ['admin']], function() { // using our 'admin' guard (which we created in auth.php)
      ...
      ;
        ...
        Route::get('delete-subscription/{id}', 'SubscriptionsController@delete'); // Delete a section in sections.blade.php
        Route::match(['get', 'post'], 'add-edit-subscription/{id?}', 'SubscriptionsController@addEdit'); // the slug {id?} is an Optional Parameter, so if it's passed, this means Edit/Update the section, and if not passed, this means Add a Section
...
Route::namespace('App\Http\Controllers\Front')->group(function() {
    ...
    Route::get('langchanger', 'IndexController@langchanger');
