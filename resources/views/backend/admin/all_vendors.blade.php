...
@section('content')
    ...
    <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
        <div class="breadcrumb-title pe-3">البائع</div>
        <div class="ps-3">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0 p-0">
                    <li class="breadcrumb-item"><a href="dashboard"><i class="bx bx-home-alt"></i></a>
                    </li>
                    <li class="breadcrumb-item active" aria-current="page">لائحت البائع</li>
                </ol>
            </nav>
        </div>
    </div>
                ...
                <table id="data_table" class="table table-striped table-bordered">
                    ...
                    @foreach($data as $item)
                        <tr>
                            ...
                            <td>
                                @if($item->status)
                                    <div class="badge rounded-pill bg-light-success text-success w-100">نشط</div>
                                @else
                                    <div class="badge rounded-pill bg-light-danger text-danger w-100">غير نشط</div>
                                @endif
                            </td>
                            <td>
                                <button type="button" class="btn btn-primary btn-sm radius-30 px-4"
                                        data-bs-toggle="modal"
                                        data-bs-target="#exampleVerticallycenteredModal-{{$item->id}}">عرض التفاصيل
                                    ...
                                    <div class="modal-dialog modal-dialog-centered">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title">معلومات البائع</h5>
                                            ...
                                            <div class="modal-body">
                                                <img src="{{url('uploads/images/profile/' . $item->photo)}}"
                                                     class="card-img-top" style="max-width: 300px; margin-left:
                                                         10px">
                                                <div class="card-body">
                                                    <h5 class="card-title">الاسم : <span style="font-weight:
                                                         lighter">{{$item->name}}</span>
                                                    </h5>
                                                    <h5 class="card-title">الحساب : <span style="font-weight:
                                                         lighter">{{$item->email}}</span>
                                                    </h5>
                                                    <h5 class="card-title">الاسم المساخدم : <span style="font-weight:
                                                         lighter">{{$item->username}}</span>
                                                    </h5>
                                                    <h5 class="card-title">العنوان : <span style="font-weight:
                                                         lighter">{{$item->address ?  : 'No address
                                                         '}}</span>
                                                    </h5>
                                                    <h5 class="card-title">رقم الهاتف : <span style="font-weight:
                                                         lighter">{{$item->phone_number ? : 'No phone number'}}</span>
                                                    </h5>
                                                    <h5 class="card-title">الحاله : <span style="font-weight:
                                                         lighter">
                                                            @if($item->status)
                                                                <span style="color: lime">نشط</span>
                                                            @else
