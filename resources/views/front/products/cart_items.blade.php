...
<div class="table-wrapper u-s-m-b-60">
    <table>
        <thead>
            <tr>
                <th class="product_cart_table">{{ __("translation.Product")}}</th>
                <th>{{ __("translation.Price")}}</th>
                <th>{{ __("translation.Quantity")}}</th>
                <th>{{ __("translation.Subtotal")}}</th>
                ...
                <tr>
                        ...
                        <div class="cart-anchor-image">
                            <a href="{{ url('product/' . $item['product_id']) }}">
                                ...
                                <h6>
                                    ...
                                    <br>
                                    {{ __("translation.Color")}}: {{ $item['product']['product_color'] }}
                                </h6>
                            </a>
                        </div>
                    </td>
                            ...
                            @if ($getDiscountAttributePrice['discount'] > 0) {{-- If there's a discount on the price, show the price before (the original price) and after (the new price) the discount --}}
                                <div class="price-template">
                                    <div class="item-new-price">
                                        <span class="real_currency">ريال</span>{{ $getDiscountAttributePrice['final_price'] }}
                                    </div>
                                    <div class="item-old-price" style="margin-left: -40px">
                                        <span class="real_currency">ريال</span>{{ $getDiscountAttributePrice['product_price'] }}
                                    </div>
                                </div>
                    ...
                    <td>
                        <div class="cart-quantity">
                            <div class="quantity">
                                <input type="text" class="quantity-text-field" id="quantity_inp" min="1"  onchange=" Math.min(Math.max(document.getElementById("quantity_inp").value, 1), 999)" value="{{ $item['quantity'] }}">
                    ...
                    <td>
                        <div class="cart-price">
                            <span class="real_currency">ريال</span>{{ $getDiscountAttributePrice['final_price'] * $item['quantity'] }} {{-- price of all products (after discount (if any)) (= price (after discoutn) * no. of products) --}}
                        </div>
                    </td>
                    <td>
        ...
        <table>
            ...
            <tbody>
                <tr>
                    ...
                    <td>
                        <span class="calc-text"><span class="real_currency">ريال</span>{{ $total_price }}</span>
                    </td>
                </tr>
                <tr>
                    <td>
                    ...
                    <td>
                        <span class="calc-text couponAmount"> {{-- We create the 'couponAmount' CSS class to use it as a handle for AJAX inside    $('#applyCoupon').submit();    function in front/js/custom.js --}}
                            ...
                            @if (\Illuminate\Support\Facades\Session::has('couponAmount')) {{-- We stored the 'couponAmount' in a Session Variable inside the applyCoupon() method in Front/ProductsController.php --}}
                                <span class="real_currency">ريال</span>{{ \Illuminate\Support\Facades\Session::get('couponAmount') }}
                            @else
