        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-12" align="center">
                    <h3>{{ __("translation.YOUR PAYMENT HAS BEEN CONFIRMED")}}</h3>
                    <p>{{ __("translation.Thanks for the Payment")}}. {{ __("translation.We will process your order very soon")}}.</p>
                    <p>{{ __("translation.Your order number is")}} {{ Session::get('order_id') }} {{ __("translation. and total amount paid is")}} <span class="real_currency">ريال</span> {{ Session::get('grand_total') }}</p> {{-- The Order Number is the order `id` in the `orders` database table. We stored the order id in Session in checkout() method in Front/ProductsController.php --}} {{-- Retrieving Data: https://laravel.com/docs/10.x/session#retrieving-data --}}
                </div>
            </div>
        </div>
    </div>
