...
class VendorController extends Controller
{
    ...
    public function vendorRegister(Request $request) { // the register HTML form submission in vendor login_register.blade.php page    
        if ($request->isMethod('post')) { // if the register form is submitted
            ...
            $vendor->mobile = $data['phone_number'];
            $vendor->email  = $data['email'];
            $vendor->vendor  = $data['vendor'];
            $vendor->vendor_type  = $data['vendor_type'];
            $vendor->birthdate  = $data['birthdate'];
            $vendor->subscription_plan  = $data['Subscription_plan'];
            ...
            date_default_timezone_set('Asia/Riyadh'); // https://www.php.net/manual/en/timezones.php and https://www.php.net/manual/en/timezones.africa.php
            ...
            $admin->mobile    = $data['phone_number'];
    ...
    public function vendorProfile($id){
        ...
        $vendor_details = VendorsBusinessDetail::where("vendor_id",$id)->first();
       
        $vendor_image = Admin::where("vendor_id",$id)->first();
        // dd($vendor_image['name']);
     
      
        if($vendor_details != null){

            return view('front.pages.vendorProfile',["products"=>$products,"vendor_details"=>$vendor_details,"vendor_image"=>$vendor_image]);
        }else{
            return abort(404);
        }
