...
class AdminController extends Controller
{
            ...
            if(\Illuminate\Support\Facades\Auth::guard('admin')->user()->type == "vendor"){
                ...
                foreach($orders as $order){
                    if($order['orders_products']){
                        ...
                        $ordersCount += 1;
                        $revenueCount += $order["grand_total"];
                    }
                }
    ...
    public function updateVendorDetails($slug, Request $request) {
         ...
         $vendorDetailsSp = \App\Models\Vendor::where('id', Auth::guard('admin')->user()->vendor_id)->first()->toArray(); // Accessing Specific Guard Instances: https://laravel.com/docs/9.x/authentication#accessing-specific-guard-instances
         ...
         if ($slug == 'personal') {
            ...
            if ($request->isMethod('post')) { // if the <form> is submitted
                ...
                if ($request->hasFile('banner_image')) { // the HTML name attribute    name="admin_name"    in update_admin_details.blade.php
                ...
                if ($request->hasFile('banner_video')) {
                    $video_tmp = $request->file('banner_video');
                ...
                }else if (!empty($data['current_vendor_video'])) { // In case the admins updates other fields but doesn't update the image itself (doesn't upload a new image), but there's an already existing old image
                    $videoName = $data['current_vendor_video'];
                } else { // In case the admins updates other fields but doesn't update the image itself (doesn't upload a new image), and originally there wasn't any image uploaded in the first place
                    $videoName = '';
                }
        ...
        if (session()->get("lang") == "en"){
            ...
            if ($vendorDetails["vendor"] == "تجاره الالكتروني"){
                $vendor = "E-Vendor";
            
                if ( $vendorDetails["vendor_type"] == "كن تاجر"){
                
                    $vendor_type = "Vendor";
                }
                if ( $vendorDetails["vendor_type"] == "حرفي"){
               
                    $vendor_type = "Craftsman";
                }
                if ( $vendorDetails["vendor_type"] == " منتجات اخري "){
                  
                    $vendor_type = "Other";
                }
           
            }
            if ($vendorDetails["vendor"] == "العلمي"){
                $vendor = "Scientific Vendor";
                
                    if ( $vendorDetails["vendor_type"] == "اكاديمي"){
                   
                    $vendor_type = "Academic";
                     }
                    if ( $vendorDetails["vendor_type"] == "مختبارات"){
                       
                        $vendor_type = "Laboratories";
                     }
            }
        ...
        if (session()->get("lang") == "ar"){
            ...
            if ($vendorDetailsSp["vendor"] == "تجاره الالكتروني"){
                $vendor = "تجار الالكتروني";
            
                if ( $vendorDetailsSp["vendor_type"] == "كن تاجر"){
                
                    $vendor_type = "تاجر";
                }
                if ( $vendorDetailsSp["vendor_type"] == "حرفي"){
               
                    $vendor_type = "حرفي";
                }
                if ( $vendorDetailsSp["vendor_type"] == " منتجات اخري "){
                  
                    $vendor_type = "اخري";
                }
           
            }
            if ($vendorDetailsSp["vendor"] == "العلمي"){
                $vendor = "علمي ";
                
                    if ( $vendorDetailsSp["vendor_type"] == "اكاديمي"){
                   
                    $vendor_type = "اكاديمي";
                     }
                    if ( $vendorDetailsSp["vendor_type"] == "مختبارات"){
                       
                        $vendor_type = "مختبارات";
                     }
            }
        ...
        if (session()->get("lang") != "ar" && session()->get("lang") != "en" && session()->get("lang") != "cn" ){
            $vendor = $vendorDetailsSp["vendor"];
            $vendor_type = $vendorDetailsSp["vendor_type"];
        ...
        return view('admin/settings/update_vendor_details')->with(compact('slug', 'vendorDetails', 'countries','vendor','vendor_type'));
    }
    ...
    public function admins($type = null) { // $type is the `type` column in the `admins` which can only be: superadmin, admin, subadmin or vendor    // A default value of null (to allow not passing a {type} slug, and in this case, the page will view ALL of the superadmin, admins, subadmins and vendors at the same time)
        ...
        $vendors = $vendors->get()->toArray(); // toArray() method converts the Collection object to a plain PHP array
$res = [];
        foreach($admins as $admin){
            $vendor = Vendor::where("id",$admin["vendor_id"])->get();

            if(count($vendor) > 0){
           
                $admin["vendor"] =  $vendor->toArray()[0];
